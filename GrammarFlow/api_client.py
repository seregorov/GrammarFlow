...
ОТРАСЛЕВОЙ ЖАРГОН И СПЕЦТЕРМИНЫ — НЕ ТРОГАЙ:
- Не подменяй узкие/цеховые слова на «похожие» словарные \
(робе ≠ роботе, проба/роба — производственная линия, не робот)
- Если слово выглядит как опечатка, но в контексте производства, склада, \
логистики, оборудования осмысленно — оставь как написал автор
- При сомнении «опечатка или термин» — оставляй оригинал, не угадывай

СОХРАНЯЙ СТРУКТУРУ:
- Абзацы, переносы строк, нумерацию и списки не меняй
- Не объединяй и не дроби абзацы, не меняй порядок предложений
- corrected_text — полный текст с исправлениями, не выжимка и не сокращение
